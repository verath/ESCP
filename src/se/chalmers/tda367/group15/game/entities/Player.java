...
import se.chalmers.tda367.group15.game.Game;
import se.chalmers.tda367.group15.game.InputHandler;
import se.chalmers.tda367.group15.game.gfx.Colours;
import se.chalmers.tda367.group15.game.gfx.Font;
import se.chalmers.tda367.group15.game.gfx.Screen;
import se.chalmers.tda367.group15.game.level.Level;
import se.chalmers.tda367.group15.game.net.packets.Packet02Move;

public class Player extends Mob {

	private InputHandler input;
	private int colour = Colours.get(-1, 000, 555, 543);
	private int scale = 1;
	...
	public void tick() {
		...
		if (xa != 0 || ya != 0) {
			...
			Packet02Move packet = new Packet02Move(this.getUsername(), this.x, this.y,
					this.numSteps, this.isMoving, this.movingDir);
	...
	public boolean hasCollided(int xa, int ya) {
		...
		if (xa != 0) {
			for (int x = xMin; x <= xMax; x++) {
				if (isSolidTile(xa, ya, x, yMin + 1)
						|| isSolidTile(xa, ya, x, yMax - 1)) {
					return true;
				}
			}
		}
		...
		if (ya != 0) {
			for (int y = yMin; y <= yMax; y++) {
				if (isSolidTile(xa, ya, xMin + 1, y)
						|| isSolidTile(xa, ya, xMax - 1, y)) {
					return true;
				}
			}
		}
