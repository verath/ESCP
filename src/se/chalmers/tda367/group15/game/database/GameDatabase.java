...
public class GameDatabase {
	...
	/**
	 * Creates a new GameDatabase and sets up the database structure if needed.
	 * 
	 * @param tempDatabase
	 ...
	 */
	public GameDatabase(boolean tempDatabase) throws ClassNotFoundException {
	 ...
	 * 
	 * @param scores
	 *            A list of InsertableScore that should be added to the
	 *            database.
	 */
	...
	public int getNumEventsByType(InsertableEvent event) {
		...
		try {
			conn = getConnection();
			stmt = conn.prepareStatement(String.format(
					"SELECT COUNT(*) FROM %s WHERE %s = ?", TABLE_EVENTS,
					EVENTS_COLUMN_TYPE));
